...
class model_data:
    ...
    def delete_reaction(self, r_id):
        if r_id in self.reactions:            
            if self.model.reactions[self.reaction_position[r_id]] not in self.objective:
    ...
    def fill_data_cobra(self):
        ...
        # objective
        self.objective = self.parse_objective(self.model.objective)
    ...
    def run_FBA(self):
        ...
        if self.model.reactions:            
            if not self.objective:
                self.update_objective(self.model.reactions[0])
            ...
            if self.sol:
                self.fluxes = self.to_dict(self.sol.x_dict)
    ...
    def set_objective(self, r_id):
        ...
        self.update_objective({self.model.reactions.get_by_id(r_id):1})
    ...
    def run_perturbation_FBA(self, boundaries, objectives = {}):
        ...
        if self.pFBA:
            print("running pFBA perturbed")
            try:
                self.perturbed_sol = cobra.flux_analysis.parsimonious.optimize_minimal_flux(perturbed_model)                              
            except Exception as e:
                print(e)
                self.perturbed_sol = False
        else:
            print("running FBA perturbed")
            self.perturbed_sol = perturbed_model.optimize()
            if self.perturbed_sol.status == 'infeasible':
                self.perturbed_sol = False

        
        if self.perturbed_sol:
            self.perturbed_fluxes = self.to_dict(self.perturbed_sol.x_dict)
    ...
    def parse_objective(self, o):
        if cobra.__version__ < '0.9.1':
            return o
        else:
            o = o.to_json()
            o_dict = {}
            for i in o['expression']['args']:
                name = i['args'][1]['name']
                value = i['args'][0]['value']
                if 'reverse' not in name:
                    react = self.model.reactions.get_by_id(name)
                    o_dict[react] = value
            return o_dict
                
    def update_objective(self, o):
        self.model.objective = o                
        self.objective = o
